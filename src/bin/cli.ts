...
program
  .name('context-cache')
  .description('Blazing-fast global context cache for AI/LLM prompts')
  .version('0.2.0');

program
  .command('init')
  .description('Create global config for current repo')
...
program
  .command('detect')
  .description('Print detected framework preset')
  .action(() => console.log(detectPreset(getRepoRoot())));

program
  .command('graph-build')
  .description('Build or rebuild SQLite graph from cache')
...
program
  .command('get-flow')
  .description('Get flow details by id or name')
  .option('--id <n>')
  .option('--name <name>')
  .action((opts: { id?: string; name?: string }) => {
    const flow = getFlow(
      getRepoRoot(),
      opts.id ? Number.parseInt(opts.id, 10) : undefined,
      opts.name,
    );
    if (!flow) {
...
program
  ...
  .description('Get community details by id or name')
  .option('--id <n>')
  .option('--name <name>')
  .option('--members', 'Include members')
  .action((opts: { id?: string; name?: string; members?: boolean }) => {
    const community = getCommunity(
      getRepoRoot(),
      opts.id ? Number.parseInt(opts.id, 10) : undefined,
      opts.name,
      Boolean(opts.members),
    );
    if (!community) {
...
program
  ...
  .description('Build focused review context for changed files')
    ...
    (opts: {
      ...
      const changed = opts.changed
        ? opts.changed
            ...
            .filter(Boolean)
        : getChangedFiles(repoRoot, opts.base);
      const context = getReviewContext(
        repoRoot,
        changed,
        Number.parseInt(opts.depth, 10) || 2,
        opts.source !== false,
        Number.parseInt(opts.maxLines, 10) || 200,
        opts.base,
      );
...
program
  .command('find-large-functions')
  .description('Find oversized functions/classes')
  ...
  .action((opts: { minLines: string; kind?: string; file?: string; limit: string }) => {
    const rows = findLargeFunctions(
      getRepoRoot(),
      Number.parseInt(opts.minLines, 10) || 50,
      opts.kind,
      opts.file,
      Number.parseInt(opts.limit, 10) || 50,
    );
...
program
  ...
  .description('Print a section from local README')
  .argument('<name>')
  .action((name: string) => {
    ...
    if (start < 0) {
      console.log(`Section not found: ${name}`);
      return;
    }
...
program
  ...
  .description('Show graph DB status and counts')
  .action(() => {
    const repoRoot = getRepoRoot();
    const s = graphStatus(repoRoot);
    if (!s.exists) {
      console.log(`No graph DB found at ${s.graphPath}\nRun: context-cache graph-build --refresh`);
      return;
    }
    console.log(
      `Graph:   ${s.graphPath}\nNodes:   ${s.nodeCount}\nEdges:   ${s.edgeCount}\nUpdated: ${s.updatedAt || '-'}`,
...
program
  ...
  .argument(
    '<pattern>',
    'callers_of|callees_of|imports_of|importers_of|tests_for|container_of|depends_on|inheritance_of|implemented_by',
  )
  .argument('<target>', 'symbol or path target')
  .option('--limit <n>', 'Result limit', '80')
  .action((pattern: string, target: string, opts: { limit: string }) => {
    ...
    for (const row of rows)
      console.log(
        `- [${row.kind}] ${row.source} -> ${row.target}${row.filePath ? ` (${row.filePath})` : ''}`,
      );
  });

program
  .command('mcp-prompts')
  .description('List built-in MCP prompt templates')
  .option('--show <name>', 'Print full prompt body')
  .action((opts: { show?: string }) => {
    if (opts.show) {
      const body = MCP_PROMPTS[opts.show.trim()];
      if (!body) {
        console.error(`Unknown: ${opts.show}`);
        process.exit(1);
      }
      console.log(`# ${opts.show}\n${body}`);
      return;
    }
    console.log('Built-in prompts:');
    for (const key of Object.keys(MCP_PROMPTS)) console.log(`- ${key}`);
  });
...
program
  ...
  .description('Risk-scored change analysis from graph')
  ...
  .action((opts: { base: string }) => {
    ...
    for (const row of rows)
      console.log(
        `[${row.risk}] ${row.filePath} | impacted=${row.impactedFiles} callers=${row.callers} tests=${row.testHits}`,
      );
  });

program
  .command('minimal-context')
  .description('Compact graph-aware context for AI entry-point calls')
  .option('--base <ref>', 'Git base ref', 'HEAD~1')
  .action((opts: { base: string }) => {
    ...
    for (const t of mc.suggestedTools) console.log(`  - ${t}`);
  });

program
  .command('graph-postprocess')
  .description('Recompute flow and community artifacts')
  .action(() => {
...
program
  ...
  .action((opts: { limit: string }) => {
    for (const r of listFlows(getRepoRoot(), Number.parseInt(opts.limit, 10) || 30))
      console.log(
        `#${r.id} ${r.name} | entry=${r.entry} files=${r.fileCount} criticality=${r.criticality.toFixed(2)}`,
      );
  });
...
program
  ...
  .action((opts: { limit: string }) => {
    for (const r of listCommunities(getRepoRoot(), Number.parseInt(opts.limit, 10) || 30))
      console.log(
        `#${r.id} ${r.name} | files=${r.fileCount} nodes=${r.nodeCount} coupling=${r.coupling}`,
      );
  });

program
  .command('architecture-overview')
  .description('Show architecture overview and coupling warnings')
  .action(() => {
    const overview = architectureOverview(getRepoRoot());
    console.log(`Communities: ${overview.communities.length}`);
    for (const c of overview.communities.slice(0, 20))
      console.log(`- ${c.name} | files=${c.fileCount} nodes=${c.nodeCount} coupling=${c.coupling}`);
...
program
  .command('embed-graph')
  .description('Compute embeddings for semantic search')
  .option('--model <name>', 'hash-v1 | ollama:<model> | openai:<model>', 'hash-v1')
...
program
  ...
  .action((symbol: string, newName: string, opts: { limit: string }) => {
    const p = refactorPreview(
      getRepoRoot(),
      symbol,
      newName,
      Number.parseInt(opts.limit, 10) || 120,
    );
    console.log(
...
program
  ...
  .description('Apply symbol rename across files')
  .argument('<symbol>')
  .argument('<newName>')
  .option('--max-files <n>', '', '200')
  .option('--yes', 'Confirm write')
  .action((symbol: string, newName: string, opts: { maxFiles: string; yes?: boolean }) => {
    const repoRoot = getRepoRoot();
    if (!opts.yes) {
      const p = refactorPreview(repoRoot, symbol, newName, 20);
      console.log(
        `Preview only. Re-run with --yes to apply.\nWould touch ~${p.filesTouched} files.`,
      );
      return;
    }
    console.log(
      `Updated files: ${applyRefactor(repoRoot, symbol, newName, Number.parseInt(opts.maxFiles, 10) || 200)}`,
    );
  });

program
  .command('generate-wiki')
  .description('Generate markdown wiki from communities')
...
program
  ...
  .description('Print generated wiki page content')
  .argument('<pageName>')
  .action((pn: string) => console.log(getWikiPage(getRepoRoot(), pn)));
...
program
  .command('refresh')
  .description('Refresh cache for current repo')
  .action(() => {
...
program
  ...
  .action(() => {
    ...
    if (!s.exists) {
      console.log(`No cache found at ${s.cachePath}\nRun: context-cache refresh`);
      return;
    }
    console.log(
      `Cache: ${s.cachePath}\nRepo: ${s.repoRoot}\nUpdated: ${s.updatedAt}\nFiles: ${s.fileCount}`,
...
program
  .command('prompt')
  .description('Print cached context to stdout')
  .option('--max-chars <n>', 'Max characters', String(DEFAULT_MAX_CHARS))
...
program
  ...
  .description('Refresh then write prompt file')
  .option('--out <file>')
  .option('--max-chars <n>', '', String(DEFAULT_MAX_CHARS))
...
program
  .command('ready')
  .description('One-step: refresh + write prompt + print next step')
  .option('--max-chars <n>', '', String(DEFAULT_MAX_CHARS))
  .option('--out <file>')
...
program
  ...
  .option('--max-chars <n>', '', String(DEFAULT_MAX_CHARS))
  .option('--no-refresh', 'Skip refresh')
  .action((opts: { maxChars: string; refresh?: boolean }) => {
    ...
    console.log(`Copied ${output.length.toLocaleString()} characters to clipboard.`);
  });

program
  .command('doctor')
  .description('Run environment and setup checks')
  .action(() => {
    const repoRoot = getRepoRoot();
    const s = status(repoRoot);
    const configPath = getConfigPath(repoRoot);
    const checks = [
      ['context-cache on PATH', check('context-cache')],
      ['git available', check('git')],
      ['global config exists', existsSync(configPath)],
      ['global cache exists', s.exists],
    ...
    if (failed > 0) {
      if (!existsSync(configPath)) console.log('  - Run: context-cache init');
      if (!s.exists) console.log('  - Run: context-cache refresh');
      process.exitCode = 1;
...
program
  ...
  .description('Estimate impacted files using import dependency graph')
  .option('--base <ref>', '', 'HEAD~1')
  .option('--changed <paths>', 'Comma-separated paths')
  .option('--depth <n>', '', '3')
  .option('--refresh', 'Refresh cache before analysis')
...
program
  ...
  .description('Find symbol definitions, references, and probable test files')
  .argument('<symbol>')
  .option('--refresh', 'Refresh cache before query')
  .option('--limit <n>', '', '120')
...
program
  .command('watch')
  .description('Watch repo and auto-refresh on changes')
  .action(() => watchRepo(getRepoRoot()));
program
  .command('cache-path')
  .description('Print cache store file path')
  .action(() => console.log(getCachePath(getRepoRoot())));
program
  .command('vscode-setup')
  .description('Add context-cache tasks to user-level VS Code tasks.json')
  .action(() => setupVscodeGlobal());
...
program
  ...
  .action(() => {
    ...
    for (const r of repos)
      console.log(`${r.alias} | ${r.path} | nodes=${r.nodeCount} edges=${r.edgeCount}`);
  });
...
program
  ...
  .argument('<alias>')
  .action((alias: string) => {
    unregisterRepo(alias);
    console.log(`Removed: ${alias}`);
  });

program
  .command('cross-repo-search')
  .description('Search across registered repositories')
  .argument('<query>')
  .option('--kind <kind>')
  .option('--limit <n>', '', '20')
  .option('--model <name>', '', 'hash-v1')
  .option('--aliases <csv>', 'Comma-separated aliases to filter')
  .action(
    (query: string, opts: { kind?: string; limit: string; model: string; aliases?: string }) => {
      ...
      for (const r of rows)
        console.log(`${r.score.toFixed(4)} | [${r.repoAlias}] ${r.qualifiedName} (${r.filePath})`);
    },
  );

program
  .command('cross-repo-impact')
  .description('Compute impact across registered repositories')
  .option('--base <ref>', '', 'HEAD~1')
  .option('--changed <paths>')
  .option('--depth <n>', '', '2')
  .option('--aliases <csv>')
...
program
  .command('parity')
  .description('Report CLI ↔ MCP tool parity')
  .action(() => {
