...
pub fn embed_graph(repo_root: &Path, model: &str) -> Result<EmbedResult> {
    ...
    let mut stmt = conn
        .prepare("SELECT qualified_name, kind, name, file_path FROM nodes WHERE kind != 'file'")?;
    ...
    while let Some(row) = rows.next()? {
        ...
        let vec = build_embedding_vector(&text, &model_name)
            .unwrap_or_else(|_| vectorize_text(&text, 64));
...
pub fn semantic_search(
    ...
    if emb_count > 0 {
        let qvec =
            build_embedding_vector(query, model_name).unwrap_or_else(|_| vectorize_text(query, 64));
