...
pub fn default_text_extensions() -> Vec<String> {
    vec![
        ".md", ".mdx", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".json", ".yaml", ".yml",
        ".txt", ".css", ".scss", ".sass", ".less", ".html", ".graphql", ".gql", ".py", ".java",
        ".kt", ".kts", ".go", ".rs", ".rb", ".php", ".cs", ".sh", ".zsh", ".bash", ".toml", ".ini",
        ".conf", ".sql", ".xml",
    ]
    .into_iter()
    .map(String::from)
...
impl CacheConfig {
    ...
    pub fn global_config_path(repo_root: &Path) -> PathBuf {
        ...
        let key = blake3::hash(repo_root.to_string_lossy().as_bytes())
            .to_hex()
            .to_string();
